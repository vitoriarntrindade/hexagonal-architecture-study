...
def client() -> Generator[TestClient, None, None]:
    ...
    within the same test, preserving state between calls.
    """
    app = create_app()
    ...
    app.dependency_overrides[get_user_by_email_use_case] = lambda: GetUserByEmailUseCase(
        user_repository=repository,
    )
...
def test_should_return_400_on_duplicate_email(client: TestClient) -> None:
    payload = {"name": "Vitória", "email": "vitoria@email.com", "password": "123456"}
...
def test_should_return_422_on_invalid_email(client: TestClient) -> None:
    response = client.post(
        "/users",
        json={"name": "Vitória", "email": "not-an-email", "password": "123456"},
    )
